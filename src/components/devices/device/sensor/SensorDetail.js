...
class SensorDetail extends Component {
  ...
  submitValue() {
    if (isNaN(this.state.newValue)) {
      ...
      let val = {
        value: Number(this.state.newValue),
      ...
      const defaultNotif = {
        ...
        action: {
          ...
          value: {
            channels: [],
            message:
              "Waziup: High temperature warning. ${id} value is ${" +
              this.props.sens.id +
              "}",
            usernames: [],
      ...
      return (
        <Container fluid={true} style={{ "padding-bottom": "100px" }}>
          ...
          <Card className="longCard">
            <Typography>
              ...
              {this.props.permission.scopes.includes("devices:update") ? (
                <div>
                  <Button
                    onTouchTap={() => this.setState({ modalAddNotif: true })}
                    variant="contained"
                    color="primary"
                  ...
                  <Button
                    onTouchTap={() => this.setState({ modalAddCalib: true })}
                    variant="contained"
                    color="primary"
                    className="topRightButton"
                  >
              ...
              <CalibrationForm
                modalOpen={this.state.modalAddCalib}
                handleClose={() => {
                  this.setState({ modalAddCalib: false });
                }}
                onSubmit={(m) => {
                  console.log(m);
                  this.props.updateSensorCalibration(
              ...
              />
              <NotifForm
                modalOpen={this.state.modalAddNotif}
                notif={defaultNotif}
                  ...
                  <Grid item xs={12}>
                    <FormControl>
                      ...
                      <Select
                        ...
                        value={this.state.timeAxis}
                        onChange={this.handleTimeAxis}
                        title="Time Axis"
                      >
          ...
          <Card className="longCard">
            ...
            <CardContent>
              <InputLabel htmlFor="numberValue">
                Push a value to this sensor:
              </InputLabel>
              <div>
                <TextField
                  id="numberValue"
...
function mapStateToProps(state, ownProps) {
  ...
  const sens = device
    ? device.sensors.find((m) => m.id == ownProps.params.sensId)
    : null;
  const notifs =
    sens && device
      ? state.notifications.notifications.filter(
          (n) =>
            n.condition.devices.includes(device.id) &&
            n.condition.sensors.includes(sens.id)
        )
      : null;
  return {
    device: device,
