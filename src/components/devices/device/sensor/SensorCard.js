    ...
    return (
      <Card className={"card " + activeStyle}>
        <SensorForm
          ...
          onSubmit={(m) => {
            this.props.updateSensor(this.props.deviceId, m);
            this.setState({ modalEdit: false });
          }}
          isEdit={true}
          sensor={sens}
              ...
              <DeleteIcon
                onClick={() => {
                  if (window.confirm("Delete sensor?"))
                    this.props.deleteSensor(this.props.deviceId, sens.id);
                }}
              />
            ) : null}
          </div>
          ...
          <div className="cardContent">
            ...
            <div className="sensValue">
              <h3>
                {" "}
                {(sens.value
                  ? JSON.stringify(sens.value.value).replace(/"/g, "")
                  : "") +
                  " " +
                  (sens.unit ? Waziup.Units.getLabel(sens.unit) : "")}{" "}
              </h3>
            </div>
          </div>
        </Link>
