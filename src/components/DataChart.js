  ...
  render() {
    ...
    let dataTriples = this.props.values.map(val => {
      ...
      switch (this.props.timeAxis) {
        case 'cloud': time = val.date_received; break; 
        case 'device': time = val.timestamp; break;
        default: time = val.date_received; break;
      }
      let unixtime = new Date(time).valueOf()
      return {id: val.device_id + " -> " + val.sensor_id, 
              time: unixtime,
              value: val.value}
    });

    //sort by time (Cloud timestamp doesn't need sorting)
    const sorted = this.props.timeAxis == 'device' ? dataTriples.sort(this.compareTime) : dataTriples;

    //grouping by ids. Ex: [{name: "MyDevice -> TC", data: [{id: XX, time: XX, value: XX}]}]
    const groupById = groupBy('id');
    const grouped = groupById(sorted);
    const grouped2 = Object.keys(grouped).map((id, index) => {return {name: id, data: grouped[id] }}) //.sort((a,b) => !this.compareTime(a,b))

    const xFormatter = (unixtime) => moment(unixtime).format('MMMM Do YYYY H:mm:ss a z');
    ...
    if(this.props.values) {
      ...
      return (
        <ResponsiveContainer  height={500} style={{margin: 30,width: '100px'}}>
          <LineChart margin={{ top: 5, right: 60, left: 0, bottom: 15 }}>
