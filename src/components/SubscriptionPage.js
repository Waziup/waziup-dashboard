...
class SubscriptionPage extends Component {
  ...
  render() {
    ...
    return (
      <div>
        <OrionParamForm action={fetchDevicesList} actionName='Subscriptions' orionService={this.props.security.userInfo.idTokenParsed.Service} orionServicePath={this.props.security.userInfo.idTokenParsed.ServicePath}/>
          {(isFetching === true) ? (<Well> Sensing Devices are being loaded. </Well> ):
            ((fetched === true) ?
             (sensingDevice.listDevices.length === 0 ? 
             (<Well> There are no Sensing Devices for the selected service and servicePath. </Well> )
             : 
             (<Well> XX There are no Sensing Devices for the selected service and servicePath. </Well>)
            ):
